...
def final_model(input_dim, units, recur_layers, filters, kernel_size, conv_stride,
    ...
    db_bn_cnn = Dropout(0.3)(bn_cnn)
    deep_rnn = Bidirectional(SimpleRNN(units, activation='relu',
                                       return_sequences=True, implementation=2, name='rnn1'))(db_bn_cnn)
    # TODO: Add batch normalization
    bn_rnn = BatchNormalization()(deep_rnn)

    for i in range(1, recur_layers):

        deep_rnn2 = Bidirectional(SimpleRNN(units, activation='relu',
                                            return_sequences=True, implementation=2, name='rnn'+str(i+1)))(bn_rnn)
        bn_rnn = BatchNormalization()(deep_rnn2)
